  ...
  localdata.value = {
    ...
    sku_list: res.result.skus.map((item) => {
      return {
        ...
        image: item.picture,
        price: item.price * 100, // 注意：价格需要*100，sku插件需要的
        sku_name_arr: item.specs.map((sku) => sku.valueName),
      ...
      <view class="preview">
        <swiper circular @change="swiperChange">
          <swiper-item v-for="item in goods?.mainPictures" :key="item">
            <image mode="aspectFill" :src="item" @tap="swiperTap(item)" />
          </swiper-item>
        </swiper>
        <view class="indicator">
          <text class="current">{{ swiperIndex + 1 }}</text>
