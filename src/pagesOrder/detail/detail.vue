...
const orderCancel = async () => {
  if (!reason.value) {
    return uni.showToast({ icon: 'none', title: '请选择取消订单原因' })
  }
  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower">
    <template v-if="order">
      ...
      <view class="detail">
        ...
        <view class="row">
          <view class="item">
            订单编号: {{ order.id }}
            <text class="copy" @tap="onCopy(order.id)">复制</text>
          </view>
    ...
    <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
      ...
      <template v-else>
        ...
        <view
          class="button delete"
          v-if="order && order.orderState >= OrderState.DaiPingJia"
          @tap="onOrderDelete"
        >
          删除订单
        </view>
